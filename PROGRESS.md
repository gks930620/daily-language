...
- 설정 중 걸린 것: 구글 콘솔의 리디렉션 URI에 `/auth/callback`이 빠져 있어
  `redirect_uri_mismatch`. 서버가 보내는 값과 문자 단위로 대조해 확인 후 해결.
- 정리: 진도 쓰기를 API 서버가 맡으면서 `scripts/lib/studylog.js`의 쓰기 함수
  (`recordStudy`·`emptyStudyLog`·`isLevel`·`STUDY_LOG_SCHEMA_VERSION`)가 아무 데서도
  안 쓰이게 됐다. 삭제하고 읽기·통계 전용 모듈로 축소(브라우저로 보내는 파일이라
  크기도 줄었다). 없어진 `state/study-log.json`·`scripts/checkin.js`를 가리키던
  주석도 정리. [A10]에서 겪은 "읽는 곳 없는 코드가 남는" 문제의 재발 방지.

## (이전) 2026-08-10 — 진도 기록 기능 가동 시작
...
`docs/`에 **JS가 처음 들어왔다** — `assets/app.js` 하나뿐이고, 학습 콘텐츠(문단·단어)는 여전히 정적이라 이 파일이 실패해도 그대로 보인다. CLAUDE.md 규칙 5를 이에 맞게 고쳤다.

- **`docs/assets/app.js`**: 로그인 상태 표시, 진도 버튼 → `PUT /study`, 내 기록 페이지 → `GET /study/me`. 인라인 스크립트 없음(마크업은 `data-*`만 두고 JS가 붙잡는다).
- **`docs/days.json`**(빌드 산출): 트랙별 콘텐츠 날짜 = **통계의 분모**. DB는 어느 날 콘텐츠가 있었는지 모른다.
- **`docs/assets/studylog.js`**(빌드가 복사): `scripts/lib/studylog.js`를 그대로 브라우저에서 재사용 — 통계 로직이 한 벌이고 기존 테스트 12개가 그대로 유효하다. API 응답 `{days:{날짜:{트랙:단계}}}`이 이 함수들에 바로 들어간다.
- **`docs/me/index.html`**: 사람마다 내용이 달라 빌드 시점에 구울 수 없다 → 껍데기만 만들고 app.js가 채운다(`renderMyPage`는 html.js에서 삭제).
- **제거**: GitHub 이슈 체크인 일체(`checkin.yml`·`scripts/checkin.js`·`state/study-log.json`) — 저장소 주인 전용이라 다중 사용자와 공존이 안 된다.
- **꺼진 상태로 안전하게 동작**: `site.js`의 `API_BASE`가 비어 있으면 진도 영역에 "설정이 아직 안 끝났습니다"만 뜨고 학습 페이지는 정상이다. SETUP.md 7단계에서 주소 한 줄 넣고 빌드하면 켜진다.
- 검증: 테스트 66 통과, 3트랙 verify 통과, 정적 서버로 `/me/`·`/days.json`·`assets/*` 200 확인, 모듈 import 실제 해석 확인, API 응답 형태로 통계 렌더 시뮬레이션.

### 남은 것

- 사용자가 SETUP.md 1~7단계 수행(구글 콘솔·Railway·API_BASE 연결) → 그러면 기능이 켜진다
- 그 뒤 실제 로그인·기록 1회 동작 확인
- (선택) 도메인을 붙이면 `AUTH_MODE=cookie`로 승격 — 기록 마이그레이션 없음
