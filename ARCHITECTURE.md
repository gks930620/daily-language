...
```
                (매일 06:00 KST, claude.ai 루틴이 저장소 클론 후 실행)
...
| `scripts/verify.js` | 오늘분 HTML 존재·개수 일치(문장 5, 단어 = selected.json 단어 수, 퀴즈 = review due 수)·settled 확인. 실패 시 exit 1 → 루틴이 커밋하지 않음. |
...
| `prompts/routine.md` | claude.ai 루틴에 붙여넣는 프롬프트 원본. |
...
| runlog의 `settled`가 멱등 키 | 루틴이 재실행·중복 실행돼도 하루 1회만 상태가 바뀐다. prepare/settle 둘 다 이 값으로 no-op 판단. |
...
| 외부 의존성 0 | 클라우드 루틴에서 npm install 불필요 → 실행 시간·실패 지점 감소. Node 22+ 내장 기능만 사용(`node --test` 글롭 인자가 22부터 안정). |
