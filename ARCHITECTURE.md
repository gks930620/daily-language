...
docs/basics/index.html         기초 교재 랜딩(로드맵 + 자동 목차 트리)
docs/basics/<원본 상대경로>.html  basics/ md를 렌더한 정적 교재(build 산출물)
docs/assets/ · docs/.nojekyll  루트 유지(공유)
basics/<lang>/…​.md             정적 기초 교재 소스(md — 매일 생성물과 별개, 손으로 편집)
...
| `scripts/build.js` | **`--lang`을 받지 않는 유일한 스크립트.** 전 언어의 `data/<lang>/*/`(content+review+selected)를 스캔 → `docs/<lang>/days/*.html` + `docs/<lang>/index.html` + 허브 `docs/index.html`을 처음부터 재생성. 이어서 **`basics/` 아래 md를 `docs/basics/`로 md→html 렌더**(로드맵 `ja/README.md`는 랜딩 `docs/basics/index.html`로 대체 렌더 + 자동 목차 트리; 허브에 📚 기초 교재 버튼). basics가 없으면 스킵. "오늘의 단어"는 selected.json 기준(없는 과거 데이터만 content.words 폴백). 단어 상태를 읽지 않는 순수 재생성. |
...
| `scripts/lib/markdown.js` | 교재용 최소 마크다운→HTML 변환기(Node 내장만). GFM 부분집합: 헤딩·파이프 표(정렬)·목록(1단 중첩)·인용·수평선·펜스 코드·문단·인라인(**굵게**·*기울임*·`코드`·[링크]). `<details>` 등 원시 HTML 줄은 패스스루, `.md` 링크는 `.html`로 재작성. `mdToHtml(md)` 하나만 export — build.js의 `buildBasics`가 쓴다. JS는 생성하지 않는다. |
| `scripts/lib/html.js` | 이스케이프 + 페이지 템플릿. JS 0줄, 네이티브 `<details>`만. **언어 분기 없음 — "reading 있으면 렌더" 규칙만.** 페이지 본문은 `renderDaySections` = 문장(문단) + 단어(클러스터)만. renderConversation/renderQuiz/renderReviewSentence는 정의는 남아 있으나 미호출(2026-07-22 회화·복습 제거, 복원용). |
...
| `docs/` | GitHub Pages 루트. **빌드 산출물** — 직접 수정 금지(`docs/basics/`의 교재 html 포함 — 소스는 `basics/`의 md). `.nojekyll` 필수. |
| `basics/<lang>/…​.md` | 정적 기초 교재 소스(md). 매일 생성물과 별개로 손으로 편집하며, build가 `docs/basics/`로 렌더한다. |
...
- **brief.json** (prepare → AI): `date`, `learner_profile`(langs.js가 단일 소스), `new_word_candidates_requested`(18 → settle이 최종 15개 선별), `known_words`, `recent_conversation_topics`(최근 14일 — 회화 제거로 현재 미사용, 필드는 유지). due 단어는 **넣지 않는다** — AI가 복습에 관여할 이유가 없다.
...
- **content.json** (AI → settle): 문단 1(문장 5로 분해)·단어 후보 20~25·회화. 최상위 `lang` 필수(--lang과 교차검증). **스키마 키는 언어 무관하게 고정**: `en` = 대상 언어 텍스트, `ko` = 한국어. requiresReading 트랙(ja-n1·ja-n2)은 sentences[]·words[]·conversation.lines[]에 `reading`(전문 히라가나) 필수, key_expressions는 선택. 검증 규칙은 validate.js 참조.
