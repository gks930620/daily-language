...
매일 아침 GitHub Actions가 이 저장소에서 **그날의 학습 페이지를 자동 생성**하고 GitHub Pages에 게시하는 학습 파이프라인입니다. 트랙은 **영어(en)·일본어 N1(ja-n1)·일본어 N2(ja-n2)** 세 개이고, 각각 독립된 단어 상태로 돌아갑니다. AI는 콘텐츠(문단·단어)만 만들고, 단어 상태 관리·HTML 빌드는 전부 Node 스크립트가 결정론적으로 처리합니다.
...
> **현재 구성(2026-07-22 확정)**: 페이지는 **오늘의 문단 + 오늘의 단어 15개**만. 회화와 복습 퀴즈는 제거했습니다(텍스트만으로는 회화 학습 효과가 약하고, 복습 방식은 다시 설계 예정 — 아래 v2 후보). SRS 간격 반복 인프라(단어 상태·박스)는 코드에 남아 있어 복습을 재도입할 때 그대로 씁니다.
...
기초가 아직 안 끝난 언어의 **정적 기초 교재**(현재 일본어)는 허브 상단의 **📚 기초 교재 버튼**으로 열람합니다(`docs/basics/` — `basics/`의 md를 build가 렌더한 것).

하루치 페이지 구성(전 트랙 공통):

1. **오늘의 문단(문장 5개)** — 서로 무관한 문장 나열이 아니라 **하나의 글에서 이어진 한 문단**(영어는 수능 한 문제 분량)입니다. 문단 원문(+글의 종류·주제 캡션, 일본어는 전문 읽기 접기)을 먼저 통으로 읽고, 아래에서 문장별로 접힌 부분을 열면 한국어 해석과 끊어읽기·구문분석이 나옵니다. 문법은 항목 나열이 아니라 그 문장을 살아 있게 만드는 포인트 중심.
...
*(회화·복습 퀴즈·복습 문장은 2026-07-22 제거 — 나중에 재설계. 코드에는 렌더러가 남아 있어 되살리기 쉽습니다.)*
