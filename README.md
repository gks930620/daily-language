...
> ⚙️ **이 기능을 켜려면 최초 1회 설정이 필요하다** → [SETUP.md](SETUP.md) (구글 OAuth + Railway, 30분)
> 설정 전까지 학습 페이지는 지금처럼 정상 동작하고, 진도 기록만 비활성 상태다.

각 학습 페이지 맨 아래에 **"오늘 얼마나 했나요?"** 버튼 세 개(조금 / 절반 / 다 함)가 있습니다.
누르면 GitHub 화면이 열리고 제목이 이미 채워져 있으니 **Submit 버튼만** 누르면 됩니다.
1~2분 뒤 기록이 반영되고, 그 이슈는 자동으로 댓글이 달린 뒤 닫힙니다.

- **내 기록**: https://gks930620.github.io/daily-language/me/ (허브 홈의 "📊 내 기록" 버튼)
  - 트랙별 **평균 진도(%)**, 기록한 날 수, 단계별 일수, **현재 연속·최장 연속**
  - 날짜별 표 — 어느 날 어느 트랙을 얼마나 했는지 한눈에(칸을 누르면 그날 페이지로)
- 잘못 눌렀으면 **다른 단계를 다시 누르면 됩니다**(나중 기록이 이깁니다).
- 지난 날짜도 그날 페이지에 들어가 기록할 수 있습니다(밀린 공부 몰아서 체크).
- 기록은 저장소(`state/study-log.json`)에 남으므로 폰·PC가 같은 기록을 보고, 브라우저를 정리해도 사라지지 않습니다.

> **회원가입은 없습니다.** GitHub Pages는 서버가 없는 정적 사이트라 로그인 기능을 붙이려면 외부 서비스가 필요합니다. 이 저장소의 주인(= 나)만 기록할 수 있게 되어 있고 — 워크플로가 **저장소 주인이 연 이슈만** 처리합니다 — 남이 이슈를 열어도 기록은 바뀌지 않습니다.
