...
2. **오늘의 단어 20개** — 뜻·예문에 더해 단어마다 **암기를 돕는 단어 지식**(💡 어원, 파생형과 뜻 변화, 혼동어와의 차이, 뉘앙스)이 붙습니다. 절반 이상은 그날의 회화 주제·문장 소재와 연관된 단어로 뽑아 같은 장면의 단어가 묶여서 외워지게 합니다(일본어는 히라가나 읽기 포함). AI가 낸 후보 25개 중 스크립트가 새 단어 20개를 선별해 싣고, 실린 단어만 복습 퀴즈로 돌아온다(중복 등으로 20개에 못 미치는 날도 있음). 단어 지식은 복습 퀴즈 정답에서도 매번 다시 보입니다.
